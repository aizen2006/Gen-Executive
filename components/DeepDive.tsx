    ...
    <section className="py-24 bg-brand-dark overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 space-y-32">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            <h3 className="text-3xl font-bold mb-6 flex items-center gap-3 text-brand-light">
            ...
            <div className="space-y-4">
              {[
                ...
                <motion.div 
                  key={i}
                  whileHover={{ x: 5 }}
                  className="p-4 rounded-xl bg-brand-surface border border-brand-border flex items-center justify-between cursor-default shadow-sm"
                >
                  <span className="font-medium text-brand-light">{chip.title}</span>
                  <span className="text-xs font-mono text-brand-muted">{chip.flow}</span>
                </motion.div>
              ))}
            </div>
          </div>

          <div className="relative h-[500px] bg-brand-surface rounded-2xl border border-brand-border p-8 flex flex-col items-center justify-center shadow-xl">
             {/* Simple Vertical Workflow Visualization */}
             <div className="relative z-10 flex flex-col items-center gap-8">
                {/* Input Node */}
                <motion.div custom={0} variants={workflowVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="w-64 p-4 bg-brand-slate rounded-lg border border-white/10 text-center relative">
                ...
                <ArrowRight className="rotate-90 text-brand-muted" />
                ...
                {/* Output Node */}
                <motion.div custom={2} variants={workflowVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="w-64 p-4 bg-brand-slate rounded-lg border border-white/10 text-center relative">
             ...
             <div className="absolute inset-0 bg-[linear-gradient(rgba(100,116,139,0.1)_1px,transparent_1px),linear-gradient(90deg,rgba(100,116,139,0.1)_1px,transparent_1px)] bg-[size:20px_20px] rounded-2xl" />
          </div>
        </div>

        {/* PART 2: AI CHATBOTS */}
        <div className="grid lg:grid-cols-2 gap-16 items-center lg:flex-row-reverse">
          <div className="order-2 lg:order-1 relative bg-brand-surface rounded-2xl border border-brand-border p-6 h-[600px] flex flex-col shadow-2xl">
            {/* Header */}
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-brand-border">
                <div className="flex gap-4">
                    <button 
                        onClick={() => setActiveTab('visitor')}
                        className={`text-sm font-medium transition-colors relative ${activeTab === 'visitor' ? 'text-brand-cyan' : 'text-brand-muted'}`}
                    >
                        Live Demo
                        {activeTab === 'visitor' && <motion.div layoutId="underline" className="absolute -bottom-5 left-0 right-0 h-0.5 bg-brand-cyan" />}
                    </button>
                </div>
                <button onClick={handleClearChat} className="text-xs text-brand-muted hover:text-red-400 flex items-center gap-1 transition-colors">
            ...
            <div ref={chatContainerRef} className="flex-1 overflow-y-auto space-y-4 p-2 custom-scrollbar scroll-smooth">
                    ...
                    >
                      ...
                      <div className={`p-3 max-w-[80%] text-sm ${
                        msg.sender === 'user' 
                          ...
                          : 'bg-brand-muted/10 text-brand-light rounded-tr-xl rounded-br-xl rounded-bl-xl'
                      }`}>
                        {msg.text}
                      </div>
                    </motion.div>
                ...
                {isTyping && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-3">
                     ...
                     <div className="bg-brand-surface p-3 rounded-tr-xl rounded-br-xl rounded-bl-xl text-brand-muted flex items-center gap-2 border border-brand-border">
            ...
            <div className="mt-4 pt-4 border-t border-brand-border flex gap-2">
                <input
                  ...
                  className="flex-1 bg-brand-dark/5 dark:bg-white/5 h-12 rounded-lg px-4 text-sm text-brand-light placeholder-brand-muted focus:outline-none focus:ring-1 focus:ring-brand-cyan transition-all border border-brand-border"
                />
                <button 
